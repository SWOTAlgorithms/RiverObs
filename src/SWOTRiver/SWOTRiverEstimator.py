...
class SWOTRiverEstimator(SWOTL2):
    ...
    def __init__(self,swotL2_file,bounding_box=None,
        ...
        try:
            ...
            try:
                ...
                self.xtrack_res = float(self.nc.range_resolution)/np.sin(np.radians(self.incidence_angle))
                ...
                try:
                    ...
                    self.pixel_area = float(self.nc.azimuth_spacing)*float(self.nc.ground_spacing)*np.ones(np.shape(self.h_noise))
        ...
        if len(self.use_heights) == len(self.class_list):
            ...
            self.h_flg=None
        #print "shape h_noise",np.shape(self.h_noise)
        #print "shape h_flg",np.shape(self.h_flg)
        ...
        for i, reach_idx in enumerate(self.reaches.reach_idx):
            ...
            if type(max_width) == type(None):
                try:
                    ...
                    max_width = self.reaches[i].metadata['Wmean']*np.ones(np.shape(self.reaches[i].x))#*2.0
                except:
    ...
    def process_reach(self,reach,reach_id,reach_idx=None,
        ...
        h_noise_ave0 = np.asarray( self.river_obs.get_node_stat('median','h_noise') )
        h_noise_std0 = np.asarray( self.river_obs.get_node_stat('std','h_noise') )
        # heights using only "good" heights
        h_noise_ave = np.asarray( self.river_obs.get_node_stat('median','h_noise','h_flg') )
        h_noise_std = np.asarray( self.river_obs.get_node_stat('std','h_noise','h_flg') )
        ...
        width_area = np.asarray( self.river_obs.get_node_stat('width_area','inundated_area') )

        # These are the values from the width database

        width_db = np.ones(self.river_obs.n_nodes,dtype=np.float64)*self.river_obs.missing_value
