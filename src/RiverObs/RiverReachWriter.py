...
class RiverReachWriter:
    ...
    def write_nodes_ogr(self,output_file,driver='ESRI Shapefile'):
        ...
        for var in self.node_output_variables:
            if  ((self.node_vars[var][0].dtype == np.float64) or
                 (self.node_vars[var][0].dtype == np.float32) ):
                self.fields[var] = 'float'
            elif ((self.node_vars[var][0].dtype == np.int32) or
                 (self.node_vars[var][0].dtype == np.int16) ):
                self.fields[var] = 'int'
            else:
                self.fields[var] = 'str'
