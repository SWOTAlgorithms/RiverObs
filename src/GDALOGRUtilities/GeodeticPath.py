...
class GeodeticPath:
    ...
    def __init__(self, lon0, lat0, lon1, lat1,ellps='WGS84',radians=False):
        ...
        self.azimuth0, self.back_azimuth1, self.distance = self.geod.inv(lon0,lat0,lon1,lat1)
    ...
    def get_path_lonlats(self,separation):
        ...
        self.lonlats = N.array(self.geod.npts(self.lon0,self.lat0,
                                             self.lon1,self.lat1,
                                             self.npts))
    ...
    def get_path(self,separation=None):
        ...
        self.heading = N.zeros(len(self.lat),dtype=self.lat.dtype)
        self.along_track_distance = N.zeros(len(self.lat),dtype=self.lat.dtype)

        for i in range(len(self.lat)-1):
            self.heading[i], back_azimuth, d = self.geod.inv(self.lon[i],self.lat[i],
...
class GeodeticPathFromPegPoint(GeodeticPath):
    ...
    def __init__(self,peg_lat,peg_lon,peg_heading,distance,peg_h=0.,mode='center',ellps='WGS84',
                 radians=False):
        ...
        if mode == 'start':
            ...
            hdg = (peg_heading + 180.)%360.
            lon0, lat0, hdg = self.geod.fwd(peg_lon,peg_lat,hdg, distance/2.)
            lon1, lat1, hdg = self.geod.fwd(peg_lon,peg_lat,peg_heading, distance/2.)

        GeodeticPath.__init__(self,lon0, lat0, lon1, lat1,ellps=ellps,radians=False)
